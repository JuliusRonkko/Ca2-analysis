function [s, d, energ] = HippoEvent_NoiseFastBestNew(region,nn,period,sss,ddd)
...
%         diffH=interp(trDifAll(i:addReg),1);
%         trDif=diff(diffH);%diff(trDifAll(i:addReg));
        ...
        intFit=hx(find(hcont>max(hcont)/2));
...
%                         plot(intHist,ny,'r-')
%                         title(num2str([mu sigma mu+sigma*(stdTh+addstd)]))
%                         figure
        %
        trDif=diff(trDifAll(i:addReg));
        [spkVetHelp, sh, dh, ener]= spkCalc_extractProva(trDif,i:i+length(trDif)-1,1,mu*1,sigma*1,(stdTh+addstd));
