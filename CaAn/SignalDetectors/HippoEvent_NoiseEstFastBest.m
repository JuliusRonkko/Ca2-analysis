function [s, d, energ] = HippoEvent_NoiseEstFastBest(region,nn,period,sss,ddd)
...
for i=regNum
    ...
    while strcmp(periodic,'yes')
        ...
        diffH=interp(trDifAll(i:addReg),4);
...
%                         plot(intHist,ny,'r-')
        ...
        [spkVetHelp, sh, dh, ener]= spkCalc_extractProva(trDif,i:i+regSiz-2,3,mu,sigma,(stdTh+addstd));
...
tr2an=dfoverf(region.traces(nn,:))*100;
ev2rem=[];
for i=1:length(s)
    if max(tr2an(s(i):d(i)))-min(tr2an(s(i):d(i)))<5
        ev2rem=[ev2rem i];
    end
end
s(ev2rem)=[];
